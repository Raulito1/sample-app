...
import { 
  ...
  Monitor, Tablet, PieChart, Users, ChevronDown, ChevronUp, 
  Terminal, Code, Calendar, Briefcase, GitCompare, ArrowLeft, ArrowRight 
} from 'lucide-react';
...
interface SidePanelProps {
  ...
  const targetStep = compareTarget === 'prev' ? prevStep : compareTarget === 'next' ? nextStep : null;

  const renderMetricComparison = (currentMetrics: any[] = [], targetMetrics: any[] = []) => {
    ...
    return (
        <div className="space-y-2">
            {allLabels.map(label => {
                ...
                return (
                    <div key={label} className="grid grid-cols-[1fr_auto_1fr] gap-4 items-center bg-white dark:bg-slate-800/40 p-3 rounded border border-slate-200 dark:border-slate-700/50">
                         {/* Current */}
                         <div className="text-right">
                            {curr ? (
                                <div>
                                    <div className="text-lg font-bold text-slate-900 dark:text-white">{curr.value}</div>
                         ...
                         <div className="text-left">
                            {targ ? (
                                <div>
                                    <div className="text-lg font-bold text-slate-600 dark:text-slate-300">{targ.value}</div>
                                    <div className="text-[10px] text-slate-400 dark:text-slate-600 uppercase">{targ.trend}</div>
                                </div>
                            ) : <span className="text-slate-400 dark:text-slate-600">-</span>}
                         </div>
                    </div>
  ...
  const renderSignatureComparison = (currentSigs: any = {}, targetSigs: any = {}) => {
      const allTypes = Array.from(new Set([
          ...Object.keys(currentSigs),
          ...Object.keys(targetSigs)
      ]));
              ...
              {allTypes.map(type => {
                  ...
                  return (
                      <div key={type} className="space-y-2">
                          <h4 className="text-xs font-mono font-bold text-cyan-600 dark:text-cyan-400/80 uppercase bg-cyan-100 dark:bg-cyan-900/10 px-2 py-1 rounded inline-block border border-cyan-200 dark:border-cyan-500/10">{type}</h4>
          ...
          {compareTarget && targetStep ? (
              ...
              <div className="space-y-8 animate-fade-in-up">
                  ...
                  <div className="grid grid-cols-2 gap-4 pb-4 border-b border-slate-200 dark:border-slate-800">
                      <div>
                          <span className="text-xs text-cyan-600 dark:text-cyan-400 font-bold uppercase mb-1 block">Current</span>
                          <h3 className="font-bold text-slate-900 dark:text-white text-sm">{step.title}</h3>
                          <p className="text-xs text-slate-500 mt-1">{step.description}</p>
                      </div>
                      <div className="text-right">
                          <span className="text-xs text-slate-500 font-bold uppercase mb-1 block">{compareTarget === 'prev' ? 'Previous' : 'Next'}</span>
                          <h3 className="font-bold text-slate-500 dark:text-slate-300 text-sm">{targetStep.title}</h3>
                          <p className="text-xs text-slate-500 dark:text-slate-600 mt-1">{targetStep.description}</p>
                      </div>
                  </div>
              ...
              <>
                ...
                {journey && (journey.valueStreamName || journey.metricsStartDate) && (
                    ...
                    {journey.valueStreamName && (
                        <div className="flex items-center gap-2 text-xs text-slate-500 dark:text-slate-400">
                            ...
                            <span>Stream: <span className="text-slate-700 dark:text-slate-300 font-medium">{journey.valueStreamName}</span></span>
                        </div>
                    )}
                    {journey.metricsStartDate && (
                        <div className="flex items-center gap-2 text-xs text-slate-500 dark:text-slate-400">
                            <Calendar size={14} className="text-slate-400 dark:text-slate-500" />
                            <span>Start Date: <span className="text-slate-700 dark:text-slate-300 font-medium">{journey.metricsStartDate}</span></span>
                        </div>
                    )}
                    </div>
                )}
                ...
                {step.metrics && step.metrics.length > 0 && (
                    ...
                    <h3 className="text-sm font-semibold text-slate-400 uppercase tracking-wide">Key Metrics</h3>
                    <div className="grid grid-cols-2 gap-4">
                        {step.metrics.map((metric, idx) => (
                            <div key={idx} className="bg-white dark:bg-slate-800/50 rounded-xl p-4 border border-slate-200 dark:border-slate-700/50 hover:border-cyan-500/30 transition-colors shadow-sm dark:shadow-none">
                                <div className="flex justify-between items-start mb-2">
                                    <span className="text-slate-500 dark:text-slate-400 text-sm">{metric.label}</span>
                                    {metric.trend === 'up' && <TrendingUp size={16} className="text-emerald-500 dark:text-emerald-400" />}
                                    {metric.trend === 'down' && <TrendingDown size={16} className="text-rose-500 dark:text-rose-400" />}
                                    {metric.trend === 'neutral' && <Minus size={16} className="text-slate-400 dark:text-slate-500" />}
                                </div>
                                <div className="text-2xl font-bold text-slate-900 dark:text-white tracking-tight">{metric.value}</div>
                            </div>
                        ))}
                    </div>
                    </div>
                ...
                <div className={`space-y-8 transition-all duration-500 ${showDrillDown ? 'opacity-100' : 'hidden opacity-0'}`}>
                        ...
                        <div className="space-y-4 animate-fade-in-up">
                            ...
                            <div className="space-y-4">
                                {Object.entries(step.signatures).map(([type, signatures]) => (
                                    <div key={type} className="bg-slate-50 dark:bg-slate-950/50 rounded-lg border border-slate-200 dark:border-slate-800 overflow-hidden">
                                        <div className="bg-slate-100 dark:bg-slate-900/80 px-4 py-2 border-b border-slate-200 dark:border-slate-800">
                                            <span className="text-xs font-mono font-semibold text-cyan-600 dark:text-cyan-300/80">{type}</span>
                                        </div>
                                        <div className="divide-y divide-slate-200 dark:divide-slate-800/50">
                                            {(signatures as any[]).map((sig, idx) => (
                                                <div key={idx} className="p-4 text-xs font-mono space-y-2">
                                                    <div className="flex items-center gap-2 mb-2">
                                                        {sig.env && <span className="px-2 py-0.5 bg-cyan-100 dark:bg-cyan-900/30 text-cyan-700 dark:text-cyan-300 rounded border border-cyan-200 dark:border-cyan-800/50">{sig.env}</span>}
                                                        {sig.action && <span className="px-2 py-0.5 bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-purple-300 rounded border border-purple-200 dark:border-purple-800/50">{sig.action.toUpperCase()}</span>}
                                                    </div>
